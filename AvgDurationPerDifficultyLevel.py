...
def main():
    ...
    input_path_S3 = config.get('INPUT_PATH', 'input_path_')
    ...
    #logger.info(f'Input Path :{input_path_S3}')
    ...
    is_beef_in_ingredients_df = is_beef_in_ingredients_df.withColumn("contains_beef",
                                                    is_beef_in_ingredients_df['ingredients_to_upper'].contains('BEEF'))

    #Filtering records with BEEF in the ingredients
    only_beef_in_ingredients_df = is_beef_in_ingredients_df.filter(
        is_beef_in_ingredients_df.contains_beef == 'true').drop('ingredients', 'ingredients_to_upper')
    ...
    total_cook_time_df = cook_time_formatted_df.withColumn('total_cook_time', (
            cook_time_formatted_df['prep_time_in_minutes'] +
            cook_time_formatted_df['cook_time_in_minutes']).cast('int'))
    ...
    avg_cooking_time_df = difficulty_level_classified_df.withColumn("avg_total_cooking_time",
                                                        ...
                                                        difficulty_level_classified_df['avg(total_cook_time)'])
                                                                    )
